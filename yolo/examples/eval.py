import numpy as np
from utils.test.datasets.voc_eval import voc_eval,voc_eval_onebox
import os
import argparse
...
def generate_aps_onebox(results_root="results", test_dataset=None):
    results_dir = results_root+"/comp4_det_test_{}.txt"
    output_path = results_root+"/perf.csv"
    anno_path = "VOCdevkit/VOC2007/Annotations/{}.xml"
    # imageset_path = "VOCdevkit/VOC2007/ImageSets/Main/test.txt"
    ...
    for class_name in class_names:
        ...
        # if os.path.exists(cache_file):
        #     os.remove(cache_file)
        rec, prec, ap = voc_eval_onebox(results_dir, test_dataset, class_name, use_07_metric=True)
        recs.append(rec)
        precs.append(prec)
        aps.append(ap)
